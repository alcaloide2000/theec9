...
**The Interrogative Challenge:** `_collect_interrogative_pairs(kyle_classes)` scans every dated Kyle class's section `content` markdown for two-column tables headed `| Statement | Question |` or `| Answer | Question |` (case-insensitive), and extracts every data row as a `{left_label, left, question, from_date, from_topic, from_section}` pair. `_render_interrogative_challenge(pairs)` groups the results by class (newest first) then by section, and renders each group's pairs back out as a plain markdown table inside an `st.expander` — a static reference list, no quiz mechanics. Only sections that actually use the pipe-table format are picked up; free-text bullet/blockquote drills (e.g. `"6. Interrogative Challenge: Which vs. Where & Whose"`, which uses `- *statement* → **question**` prose instead of a table) are intentionally skipped rather than parsed with a fragile regex. New tables are picked up automatically — no code change needed, only adding the section to `class_cache.json`.
